...
void TimerQueue::CancelTimer(Timer* timer_ptr) {
  if (!timer_ptr || !timer_ptr->queue_) return;
...
void TimerQueue::ScheduleTimer(Timer* timer_ptr, uint64_t timer_interval,
                               int64_t time_delay) {
  ...
  while (last_update_time_ < delta_time) {
    ...
    list_.for_each([&](Timer * timer, size_t iter)->bool {
      if (timer) {
        ...
        if (timer) {
          int64_t delay = timer->GetNextHandleTime() -
                          (timer->GetInterval() + this->GetLastUpdateTime());

...
void ConnManager::AddHandler(const HandlerPtr& ptr) {
  int fd = ptr->fd();
  if (fd >= (int)this->handlers_.size()) this->handlers_.resize(fd << 1);
  this->handlers_[fd] = ptr;
}

void ConnManager::ResetHandler(int fd) {
  if (fd > 0 && fd < (int)this->handlers_.size()) this->handlers_[fd].reset();
}
...
void ConnManager::Update() {
  ...
  if (this->add_callback_ && add_set.size()) this->add_callback_(add_set);

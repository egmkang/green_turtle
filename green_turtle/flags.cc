...
static inline std::vector<std::string>::const_iterator FindFlag(
    const std::vector<std::string>& argv, const std::string& flag) {
...
std::string CommandFlags::GetFlags(const std::string& flag) {
  ...
  if (iter != this->argv_.end() && iter + 1 != this->argv_.end() &&
      *(iter + 1)->begin() != '-') {
    return *(iter + 1);
  }
  return std::string();

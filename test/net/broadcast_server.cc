...
class PrintMessageCount : public Timer {
 public:
  void OnTimeOut(uint64_t current_time) {
    ...
    std::cout << "Message Count : " << count << " , cost time : " << time_stamp
              << "ms , "
...
static BroadCastTask* GetTask(int fd) {
  std::unordered_map<int, BroadCastTask*>::const_iterator iter =
      task_manager_.find(fd);
  if (iter != task_manager_.end()) return iter->second;
  return nullptr;
}
...
std::shared_ptr<EventHandler> NewEventHanlder(int fd, const AddrInfo& addr) {
  std::shared_ptr<BroadCastTask> pTask =
      std::make_shared<BroadCastTask>(fd, addr);
...
int main() {
  ...
  std::shared_ptr<TcpAcceptor> acceptor = std::make_shared<TcpAcceptor>(
      "127.0.0.1", 10001, std::bind(&NewEventHanlder, std::placeholders::_1,
                                        std::placeholders::_2));
  acceptor->SetWindowSize(32 * 1024, 32 * 1024);

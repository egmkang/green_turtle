...
class EchoMessage : public Message {
 public:
  EchoMessage(const char* data) : data_(data) {}
  EchoMessage(const char* data, size_t len) : data_(data, data + len) {}
...
static char* NewEchoString(int& send_times_) {
  ...
  snprintf(str, 100, "this is echo string sent by client, %d times",
           send_times_);
  send_times_++;
  return str;
}
...
class EchoClient : public TcpClient {
 public:
  EchoClient(const std::string& ip, unsigned short port)
      : TcpClient(ip, port), send_times_(0) {}
